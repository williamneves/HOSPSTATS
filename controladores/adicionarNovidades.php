...
$_UP['pasta'] = $_SERVER['DOCUMENT_ROOT'] . '/images/novidades/';
...
else
{
	...
	else
	{
		...
		$nome_final = $foto['name'];
	}
	// Depois verifica se é possível mover o arquivo para a pasta escolhida
	if (move_uploaded_file($foto['tmp_name'], $_UP['pasta'] . $nome_final))
	{
		// Upload efetuado com sucesso, exibe uma mensagem e um link para o arquivo
		echo "Novidade cadastrada com sucesso!";

		header('Location: ../painel.php?pagina=novidades');

		// Mando para o Banco de dados ;)
		$query = $Conexao->query("INSERT INTO novidades (Titulo,Foto,Descricao,URL) VALUES ('".utf8_decode($dados['Titulo'])."','".$nome_final."','".utf8_decode($dados['Descricao'])."','".utf8_decode($dados['URL'])."')");
