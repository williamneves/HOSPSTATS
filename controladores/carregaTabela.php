...
{
	$SQL = "SELECT CABSERV.NUMREQSERV, CABSERV.NOMEPAC, TABINTSV.DESCINTSV, ARQATEND.CODPLACO, 
		CASE ITMSERV.POSICAO 
		WHEN '1' THEN 'SOL' WHEN '2' THEN 'S/R' WHEN '3' THEN 'S/R' WHEN '4' THEN 'C/R' WHEN '5' THEN 'C/R' WHEN '6' THEN 'CAN' END AS POSICAO, CABSERV.DATASOL, 
		CASE TABINTSV.GRUPOINTSV WHEN '1001' THEN 'BIOQUIMICA' WHEN '1002' THEN 'HEMATOLOGIA' WHEN '1018' THEN 'APOIO' WHEN '1003' THEN 'IMUNOLOGIA' WHEN '1007' THEN 'URINALISE' WHEN '1005' THEN 'COPROLOGIA' WHEN '1007' THEN 'MICROBIOLOGIA' WHEN '1009' THEN 'DIVERSOS' END AS BANCADA FROM CABSERV
		...
		AND ARQATEND.CARATER IN ('02', 'UR') AND DATE_PART('day', date('2016-09-09')::timestamp - date(cabserv.datasol)::timestamp) * 24 + DATE_PART('hour', date(current_date)::timestamp - date(cabserv.datasol)::timestamp) <= '".$horario."' Order by tabintsv.grupointsv,cabserv.datasol";
}
